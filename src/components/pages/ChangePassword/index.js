...
import { API_CHANGE_PASSWORD_FORGOT } from '../../API'
...
function ChangePassword({ languageSelected, setProgress }) {
    ...
    const tokenSaved = localStorage.getItem('tokenForgot')
    ...
    const handleClickResetPassword = async () => {
        if (password.length < 6) {
            ...
            await axios.post(`${API_CHANGE_PASSWORD_FORGOT}?token=${token}&newPassword=${password}`).then(() => {
    ...
    return (
        <>
            {token === null || token !== tokenSaved ? <Navigate to='/' /> :
